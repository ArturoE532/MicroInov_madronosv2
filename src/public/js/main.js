...
function toggleopc(){
    ...
    for(i = 0; i < opcContent.length; i++){
        opcContent[i].className = 'opc__content opc__close'
    }
    if(itemClass === 'opc__content opc__close'){
        this.parentNode.className = 'opc__content opc__open'
    }
}

opcHeader.forEach((el) =>{
    el.addEventListener('click', toggleopc)
})
